...
import gov.sls.ods.service.Ods376iService;
...
@Slf4j
@Controller
@RequestMapping("ODS376I")
//@PreAuthorize("hasAuthority('AUTHORITY_ODS376I')")
public class Ods376iResource {
    ...
    @Autowired
    private Ods376iService ods376iService;
    ...
    final static String ODS376_USERTYPE = "B";
    final static String ODS376_PACKAGE_CODE = "O1504@ALL";
    
    final static String YEAR_FORMATE_PATTERN = "^(19|20|21)[0-9]{2}$";
    
    /** 營業人個別化主題-營業人營所稅統計查詢。查詢營業人自己之營所稅營業淨利、營業收入淨額統計資料。
...
//        ban = BanMask.getInstance().process(ban); // ban 加密
        Ods376iQueryDto ods376iQueryDto = new Ods376iQueryDto();
        ods376iQueryDto.setV(version);
        ods376iQueryDto.setBan(ban);
        ods376iQueryDto.setDataYrS(dataYrS);
        ods376iQueryDto.setDataYrE(dataYrE);
        if ( ! AbnormalResourceAccessUtil.isAbnormalAccess(request, appID) ) {
            ...
            if(version == null || version.equals("") || appID == null || appID.equals("") 
                ...
                ods376iQueryDto.setCode("903");
                ods376iQueryDto.setMsg("參數錯誤");
            }else if(!dataYrS.matches(YEAR_FORMATE_PATTERN) || !dataYrE.matches(YEAR_FORMATE_PATTERN)
                    || !ods376iService.yearVerify(dataYrS, dataYrE)){
                ods376iQueryDto.setCode("903");
                ods376iQueryDto.setMsg("參數錯誤");
            }else if(!apiValidator.validationApiId(appID)) {
                ods376iQueryDto.setCode("998");
                ods376iQueryDto.setMsg("APPID錯誤");
            }else if(!apiValidator.validationAppIdAndBan(appID, ban)) {
                ods376iQueryDto.setCode("998");
                ods376iQueryDto.setMsg("APPID申請者統編與傳入BAN不符");
                AbnormalResourceAccessUtil.updateAbnormalAccess(request, appID);
            }else if(!ValidateCode.ACCOUNT_SUCCESS.equals(validateAccount)) {
                ods376iQueryDto.setCode("999");
                ods376iQueryDto.setMsg(validateAccount.getMsg());//直接抓validateCode
                AbnormalResourceAccessUtil.updateAbnormalAccess(request, appID);
            }else {
                ods376iQueryDto = ods376iService.query(ods376iQueryDto);
                if(ods376iQueryDto.getDetails().size()==0) {
                    ods376iQueryDto.setCode("901");
                    ods376iQueryDto.setMsg("查無資料");
                }else {
                    ods376iQueryDto.setCode("200");
                    ods376iQueryDto.setMsg("執行成功");
                }
            }
        } else {
            ods376iQueryDto.setCode("998");
            ods376iQueryDto.setMsg("異常登入次數超過5次，請24小時後再登入");
        }
        
        // 產生 API log
        String userId = ODS376_USERTYPE + "_" + ban  + "_" + account;
        String packageId = odsPackageRepository.findIdByCode(ODS376_PACKAGE_CODE);
        Integer packageVer = 1;//個別化主題預設為1
        odsApiLogService
                .createLog("009", request.getRemoteAddr(), new Date(), packageId, packageVer,
                        request.getQueryString(), ods376iQueryDto.getCode(), appID, userId);
        
        JsonConfig jsonConfig=new JsonConfig();
        jsonConfig.setExcludes(new String[] {"ban"});
        return (JSONObject) JSONSerializer.toJSON(ods376iQueryDto,jsonConfig);
    }
